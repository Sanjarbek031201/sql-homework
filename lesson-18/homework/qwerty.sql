...
(20, 10, 1, '2024-12-25'),
(21, 1, 1, '2024-04-20');*/
--1.Create a temporary table named MonthlySales to store the total quantity sold and total 
--revenue for each product in the current month.
/*select * from sales
select * from products*/
create table #MonthlySales1 (product_id int, quantity int,price int, revenue DECIMAL(10, 2));
SELECT 
    product_id,
...
FROM Sales
WHERE 
    MONTH(sale_date) = MONTH(GETDATE()) AND
    YEAR(sale_date) = YEAR(GETDATE())
GROUP BY product_id;
...
SELECT 
    ...
    p.ProductName,
    p.Category,
    SUM(s.SalesAmount) AS TotalQuantitySold
FROM Products p
...
BEGIN
    ...
    SELECT @TotalRevenue = SUM(s.SalesAmount * s.SalesAmount)
    FROM Sales s
...
( select p.productname,
 sum(s.SalesAmount) as Totalquantity,
 sum(s.SalesAmount*s.Productid) as TotalRevenue
 from products p
 join sales s on p.productid=s.productid
...
SELECT
    RowNumber,
    -- Fill down the last non-null Workflow seen so far
